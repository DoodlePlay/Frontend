...
const Toolbar = () => {
  ...
  return (
    <div className="max-w-[150px] p-[20px] bg-primary-default flex gap-[20px] flex-col justify-between items-center rounded-lg shadow-lg border-[4px] border-black">
      {/* 상단 도구 아이콘 섹션 */}
      <div className="flex w-full flex-row flex-wrap gap-x-[10px] gap-y-[20px] justify-between items-center">
        <div className="flex w-3 h-3 bg-black rounded-full cursor-pointer"></div>
        <div className="flex w-4 h-4 bg-black rounded-full cursor-pointer"></div>
        <div className="flex w-5 h-5 bg-black rounded-full cursor-pointer"></div>
      </div>
      ...
      <div className="flex w-full flex-row flex-wrap gap-x-[10px] gap-y-[20px] justify-between">
        {colors.map((color, index) => (
          <div
            key={index}
            className={`max-w-1/2 rounded-[5px] p-[10px] cursor-pointer hover:bg-primary-300`}
          >
            <svg
              width="25"
              height="26"
...
// ToolItem 컴포넌트 (아이콘마다 서로 다른 SVG 사용)
...
const ToolItem = ({ icon, selected, onClick }: ToolItemProps) => {
